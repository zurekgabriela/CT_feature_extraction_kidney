...
for i = 1:length(CT)   
    ...
    for radius = 11 : radius_size
        % 3D Histogram Zorientowanych Gradientów
        tic
        CT(i).HOG = extract3DHOG( single(CT(i).norm), CT(i).class, [radius radius]);
        toc
...
feature_length = 18;
response = samples(:, 4);
% pierwszy porownywany promien
first_radius = 11;
smallest_error = 1;
% pierwsza kolumna w wektorze cech
first = 6;

for i = first_radius : radius_size
