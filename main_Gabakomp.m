...
for i = 1:length(CT)   
    ...
    [CT(i).LBP] = extractLBP( CT(i).norm, CT(i).class, 2 ); 
    ...
    CT(i).HOG = extract3DHOG( single(CT(i).norm), CT(i).class, [5 5]);
    ...
    validation_error.LoG = classify( CT(i).samples, classifyData );
    sprintf('klasyfikacja zosta³a wykonana %s',CT(i).patient)
...
% CT = rmfield(CT, 'class'); CT = rmfield(CT, 'norm');
...
validation_err.LBP = 1 - x/size(SVMclass,1);
clear i; clear x;
